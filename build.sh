...
function build() {
  ...
  echo "Building AWS_CONNECT_VM_SERVERLESS"
  cd ./$AWS_CONNECT_VM_SERVERLESS || exit

  echo "Build JavaScript Lambda handlers"
  eval "npx webpack"

  echo "Build Java Lambda handlers"
  eval "mvn clean package"

  echo "Build Serverless package"
  eval "npx serverless package --region $2 --stage $1"
  
  echo "Building AWS_CONNECT_VM_SERVERLESS completed"
  cd ..
  ...
  cp "./$AWS_CONNECT_VM_SERVERLESS/.serverless/cloudformation-template-update-stack.json" "$AWS_CONNECT_VM_SERVERLESS_DIST/templates/aws-connect-vm.template.yaml"

  eval "python ./tools/transform.py --template "$AWS_CONNECT_VM_SERVERLESS_DIST/templates/aws-connect-vm.template.yaml" --save "$AWS_CONNECT_VM_SERVERLESS_DIST/templates/aws-connect-vm.template.yaml" --zip $AWS_CONNECT_VM_SERVERLESS/aws-connect-vm.zip --jar $AWS_CONNECT_VM_SERVERLESS/aws-connect-vm-java.jar"
